...
GROUP BY user_name;
-- GROUP BY는 동명이인을 하나의 집단으로 보기때문에 문제가 되고,
-- 하나의 테이블에 전체 평균을 넣을수 없다.
-- 서브쿼리를 이용하면 된다.
...
-- FROM 절 SUB QUERY를 활용한 범위좁히기 개념 알기
-- FROM절 다음에는 테이블명만 올 수 있는게 아니고, 데이터 시트 형식을 갖춘 자료라면, 모든 형태의 자료를 적을 수 있다.
...
-- 서브쿼리의 실제 활용 : 최지선보다 키 큰 사람을 한 줄로 결과얻기
-- 기존방식으로 처리하는 경우
-- 1. 최지선의 키를 WHERE절을 이용하여 확인
...
-- 개선방안 : WHERE절에, user_height > 170(서브쿼리로 최지선의 키 구하기);

-- 1. 최지선의 키만 나오는 구문 먼저 적기
SELECT user_height FROM user_tbl WHERE user_name = '최지선';

-- 2. SELECT * FROM user_tbl WHERE user_height > (1의 쿼리문);
SELECT * FROM user_tbl WHERE user_height > (SELECT user_height FROM user_tbl WHERE user_name = '최지선');

-- 평균보다 체중이 낮은 사람만 조회하는 서브쿼리문 만들기
SELECT * FROM user_tbl WHERE user_weight < (SELECT AVG(user_weight) FROM user_tbl);

-- 지역이 경기인 사람들 중 키가 제일 큰사람보다 키가 더 큰 사람 조회하기
SELECT * FROM user_tbl
...
SELECT * FROM user_tbl  # 전체 테이블에서
	WHERE user_address IN # user_address가 속하는
		(SELECT user_address FROM user_tbl # 키가 큰 지역
		 WHERE user_height > (SELECT AVG(user_height) FROM user_tbl) # 전체 키평균보다 키가 큰 지역
