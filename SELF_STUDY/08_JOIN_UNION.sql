/*
	JOIN
    2개 이상의 테이블을 결합, 여러 테이블에 나눠 삽입된 연관된 데이터를 결합해주는 기능
    같은 내용의 컬럼이 존재해야만 사용할 수 있다.
    
    SELECT 테이블1.컬럼1, 테이블1.컬럼2 , ... 테이블2.컬럼1, 테이블2.컬럼2, ...
    FROM 테이블1 JOIN 구문 테이블2
    ON 테이블1.공통컬럼 = 테이블2.공통컬럼;
    
    WHERE 구문을 이용해 ON절로 합쳐진 결과 컬럼에 대한 필터링이 가능하다.
*/

-- 예제 데이터를 삽입하기 위해 테이블 2개 생성(외래키 없이)
...
SELECT user_tbl.user_name, user_tbl.user_address, user_tbl.user_num,
    ...
    ON user_tbl.user_num = buy_tbl.buy_num;

-- 테이블명 별칭으로 줄이기
-- FROM절에서 테이블명을 지정할때, FROM 테이블명 별칭1 JOIN 테이블명 별칭2
-- 형식을 사용하여 별칭을 테이블명 대신 쓸 수 있다.
...
SELECT u.user_name, u.user_address, u.user_num,
    ...
    ON u.user_num = b.buy_num;
    
-- LEFT JOIN과 RIGHT JOIN은 JOIN절 왼쪽이나 오른쪽 테이블은 전부 데이터를 남기고,
...
-- 결과 ROW의 개수는 테이블 1의 ROW개수 * 테이블2의 ROW개수가 된다.

-- 테스트 코드를 돌려보고, 예제 테이블 다시 만들기
SELECT * FROM
	user_tbl, buy_tbl; -- CROSS JOIN 구문 생략 가능
    
-- user_tbl 10개 행, buy_tbl 3개 행 = 크로스조인 결과 행은 30개
SELECT COUNT(*) FROM 
...
/*
    ...
    예시로는 사원 목록 중 자기 자신과 직속상사를 나타내거나 게시판에서 원본글 답변글을 나타내는 경우
...
-- SELF JOIN을 이용해 직원이름과 상사이름이 같이 나오게 만들기
-- SELF JOIN은 테이블 자기 자신을 자기가 참조하도록 만들기 때문
-- JOIN시, 테이블명은 좌, 우 같은 이름으로, AS를 이용해서 부여한 별칭은 다르게 해서
-- 좌측과 우측 테이블을 구분하면 된다.
SELECT
	l.staff_name, r.staff_name
FROM
	staff AS l INNER JOIN staff AS r
ON
	l.staff_num = r.staff_supervisor;

-- UNION은 SELECT문 두 개를 위 아래로 배치해서 양쪽 결과를 붙여준다.
-- 수직적 확장을 고려할때 주로 사용한다.
-- 고양이 테이블 생성
CREATE TABLE cat(
...
-- MySQL은 FULL OUTER JOIN을 UNION을 이용해서 한다.
-- LEFT 조인 구문 UNION RIGHT 조인구문 순으로 작성한다.
