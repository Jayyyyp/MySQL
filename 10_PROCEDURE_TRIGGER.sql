/* MySQL에서도 프로그래밍이 가능
   Java, Python 등과 달리 제약이 많지만 반복 작업을 줄여줄 수 있다는 점에서 많이 사용*/
   
   /* <MySQL에서 변수 사용하기>
	  변수를 지정할 때는 SET @변수명 = 값; 문법을 사용
   */
   SET @myVar1 = 5;
   SET @myVar2 = 3;
   SET @myVar3 = 4.25;
   SET @myVar4 = '유저명';
   SET myVar5 = 50; -- 변수명 왼쪽에 @를 작성안하면?
   
   -- 출력은 SELECT @변수명; 을 사용
   SELECT @myVar1 + @myVar2 - @myVar3;
   
   -- 테이블을 조회하던 SELECT 구문처럼 ,로 여러 데이터 반환 가능
   SELECT @myVar4, user_name FROM user_tbl;
   
   /* PREPARE 구문
	  PREPARE 구문에 가변적으로 입력되는 문장욧의 자리에 ? 를 입력하고 그 자리를 EXECUTE 구문으로 채움
...
SELECT user_name, user_height FROM user_tbl LIMIT 5;

EXECUTE myQuery USING @myVar5;

-- 조건문 사용하기
/* IF(수식, 참일때 리턴, 거짓일때 리턴)
	삼항 연산자와 비슷하게 판단 - 조건문? 참:거짓
*/
...
-- 프로시저 생성하기 프로시저의 이름은 getOverAvgHeight()
-- 평균보다 키 큰 사람 전체 목록을 호출하는 프로시저
DELIMITER $$
...
-- 프로시저 삭제시 DROP PROCEDURE 프로시저명; -- ()는 쓰지 않음
-- 특정 프로시저는 DB에 소속된 것이기 때문에 반드시 DB가 먼저 선택되어야 명령 수행
...
CREATE PROCEDURE getScore()
    ...
    DECLARE ranking CHAR;
		...
		SELECT CONCAT('취득점수 : ', points, ', 학점 : ', ranking);
...
DROP PROCEDURE getScore;
   
/* SQL에서 WHILE 구문 작성
	WHILE(조건식) DO
		실행문..
	END WHILE;
    조건식이 특정 조건을 만족시키지 못하면 WHILE구문을 탈출하도록 함
*/
DELIMITER $$
...
DELIMITER //
	CREATE TRIGGER testTrg
        ...
        FOR EACH ROW
	BEGIN
		# OLD.컬럼명을 적으면 해당 삭제 row의 데이터를 조회할 수 있다.
